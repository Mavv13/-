...
def make_matrix(getList):
    sortedList = sorted(getList, key=lambda tup: tup[0])
    max_row = 0
    index_list = []
    index = []
    ...
    temp = [[{'value': ''} for count in range(max_col)]for count in range(max_row)]

    res = []
    res.extend(temp)

    for elem, replacement in zip(index_list, sortedList):
